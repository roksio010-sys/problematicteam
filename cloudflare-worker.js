...
async function handle(request, env) {
  ...
  if (path === '/visit') {
    ...
    const clientCertSerial = /^[A-Fa-f0-9]{1,128}$/.test(clean(request.headers.get('X-PMT-Client-Cert-Serial'), 128)) ? clean(request.headers.get('X-PMT-Client-Cert-Serial'), 128).toUpperCase() : '';
    ...
    if (!blocked && clientCertSerial) {
      const row = await env.DB.prepare('SELECT 1 FROM blocked_client_certs WHERE serial = ? LIMIT 1').bind(clientCertSerial).first();
      blocked = !!row;
    }
    ...
    try {
      ...
      await env.DB.prepare(`INSERT INTO visits
        (visited_at, ip, country, site_origin, page, device_type, device_model, os, browser, browser_version, screen_resolution, device_pixel_ratio, language, locale, timezone, hour_cycle, hardware_threads, device_memory_gb, battery_level, battery_charging, referrer, visitor_id, client_cert_serial, click_count, max_scroll)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)` )
        .bind(now, ip, code, origin, page, deviceType, deviceModel, os, browser, browserVersion, screenResolution, devicePixelRatio, language, locale, timezone, hourCycle, hardwareThreads, memoryValue, batteryValue, batteryCharging, referrer, visitorId, clientCertSerial, clicks, scroll).run();
  ...
  if (path === '/api/edge-check' && request.method === 'GET') {
    const secret = request.headers.get('X-PMT-Edge-Secret') || '';
    if (!env.EDGE_SHARED_SECRET || secret !== env.EDGE_SHARED_SECRET) return reply({ error: 'forbidden' }, 403);
    const serial = String(url.searchParams.get('serial') || '').trim().toUpperCase();
    if (!/^[A-F0-9]{1,128}$/.test(serial)) return reply({ blocked: false });
    const row = await env.DB.prepare('SELECT 1 FROM blocked_client_certs WHERE serial = ? LIMIT 1').bind(serial).first();
    return reply({ blocked: !!row });
  }
  ...
  if (path === '/api/cert-block' && request.method === 'POST') {
    let data;
    try { data = await bodyJSON(request, 4096); } catch { return reply({ error: 'invalid-body' }, 400); }
    const serial = typeof data.serial === 'string' && /^[A-Fa-f0-9]{1,128}$/.test(data.serial) ? data.serial.toUpperCase() : '';
    if (!serial) return reply({ error: 'invalid-cert-serial' }, 400);
    if (data.blocked) await env.DB.prepare('INSERT OR IGNORE INTO blocked_client_certs (serial, blocked_at) VALUES (?, ?)').bind(serial, now).run();
    else await env.DB.prepare('DELETE FROM blocked_client_certs WHERE serial = ?').bind(serial).run();
    return reply({ ok: true, blocked: !!data.blocked });
  }
  if (path === '/api/visits' && request.method === 'GET') {
    ...
    const result = await env.DB.prepare(`SELECT v.id, v.visited_at, v.ip, v.country, v.site_origin, v.page, v.device_type, v.device_model, v.os, v.browser, v.browser_version, v.screen_resolution, v.device_pixel_ratio, v.language, v.locale, v.timezone, v.hour_cycle, v.hardware_threads, v.device_memory_gb, v.battery_level, v.battery_charging, v.referrer, v.visitor_id, v.client_cert_serial, v.click_count, v.max_scroll, CASE WHEN b.visitor_id IS NULL THEN 0 ELSE 1 END AS blocked, CASE WHEN bc.serial IS NULL THEN 0 ELSE 1 END AS cert_blocked
      FROM visits v LEFT JOIN blocked_visitors b ON b.visitor_id = v.visitor_id LEFT JOIN blocked_client_certs bc ON bc.serial = v.client_cert_serial
