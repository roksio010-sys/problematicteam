...
async function cleanup(env, now = Date.now()) {
  if (!env.DB) return;
  await env.DB.batch([
    env.DB.prepare('DELETE FROM visits WHERE visited_at < ?').bind(now - RETENTION_MS),
...
async function handle(request, env) {
  ...
  if (path === '/api/visits' && request.method === 'GET') {
    ...
    const result = await env.DB.prepare(`SELECT id, visited_at, ip, country, site_origin, page, device_type, os, screen_resolution, device_pixel_ratio
      FROM visits WHERE visited_at >= ? AND id < ? ORDER BY id DESC LIMIT 101`)
      .bind(now - RETENTION_MS, before).all();
    const rows = result.results || [];
    const more = rows.length > 100;
    const visits = rows.slice(0, 100);
    return reply({ visits, next: more ? visits[visits.length - 1].id : null, retentionDays: 7 });
  }
